...
//
// 동작: 텍스트 입력 → 백엔드 서버(/api/generate)에 요청
//       → 서버가 Claude를 불러 결과를 돌려줌 → 화면에 표시
//
// 중요: 여기서는 Claude를 직접 부르지 않는다.
//       우리 백엔드 서버를 부르고, 서버가 대신 Claude를 부른다.
//       (API 키를 브라우저에 노출하지 않기 위해서)
// ============================================================
  ...
  return (
      ...
      >
        <div style={{ maxWidth: 820, margin: "0 auto" }}>
          {/* 제목 */}
          <h1 style={{ fontSize: 26, color: "#fff", margin: 0 }}>
            <span style={{ color: "#7c9cff" }}>{">"}</span> 개발자 AI 도구상자
          </h1>
          <p style={{ color: "#8b929e", fontSize: 13, marginTop: 6 }}>
            자주 쓰는 개발 작업을 AI에게 맡기세요.
          </p>
          ...
          {/* 안내 문구 */}
          <p style={{ color: "#8b929e", fontSize: 13, marginBottom: 10 }}>
            {activeTool.hint}
          </p>
          ...
          >
            <button
                ...
                disabled={loading}
                style={{
                  background: loading ? "#3a4356" : "#7c9cff",
                  color: loading ? "#8b929e" : "#0f1117",
                  border: "none",
                  borderRadius: 8,
                  padding: "10px 20px",
                  fontSize: 14,
          ...
          {/* 결과 */}
            {output && (
                <div style={{ marginTop: 24 }}>
                    <span style={{ color: "#8b929e", fontSize: 13 }}>결과</span>
                    {/* 결과를 감싸는 박스 */}
                    <div
                        style={{
                            background: "#161922",
                    ...
                    >
                        {/* 마크다운을 실제 HTML로 렌더링 */}
                        <ReactMarkdown
                            remarkPlugins={[remarkGfm]} // 표 등 지원 활성화
                            components={{
                                // 코드 블록/인라인 코드 스타일
                                    ...
                                    return (
                                        <code
                                            style={{
                                                ...
                                                border: "1px solid #232733",
                                                borderRadius: 4,
                                                padding: isBlock ? "12px 14px" : "2px 6px",
                                ...
                                // 링크가 새 탭에서 열리도록
                                a: ({ children, href }) => (
                                    <a href={href} target="_blank" rel="noreferrer" style={{ color: "#7c9cff" }}>
                                        {children}
                                    </a>
                                ),
                            }}
